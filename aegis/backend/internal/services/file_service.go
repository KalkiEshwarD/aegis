...
func (fs *FileService) ensureBucketExists() {
	...
	if err != nil {
		...
		log.Printf("MinIO may not be properly configured. File operations will use mock storage.")
		return
	}
...
func (fs *FileService) UploadFile(userID uint, filename, mimeType, contentHash, encryptionKey string, fileData io.Reader, sizeBytes int64, folderID *uint) (*models.UserFile, error) {
	db := database.GetDB()

	// Check for existing user file
	var userFiles []models.UserFile
	err := db.Where("user_id = ?", userID).Preload("File").Find(&userFiles).Error
	if err != nil {
		return nil, fmt.Errorf("database error fetching user files: %w", err)
	}
...
func (fs *FileService) MoveFile(userID, fileID uint, newFolderID *uint) error {
	db := database.GetDB()

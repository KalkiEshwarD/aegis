...
// AllowedEmails is the resolver for the allowed_emails field.
func (r *fileShareResolver) AllowedEmails(ctx context.Context, obj *models.FileShare) ([]string, error) {
	if obj == nil {
		return nil, fmt.Errorf("FileShare object is nil")
	}
	if obj.AllowedEmails == "" || obj.AllowedEmails == "[]" {
		return []string{}, nil
	}
	var emails []string
...
func (r *mutationResolver) RestoreFolder(ctx context.Context, folderID string) (bool, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return false, fmt.Errorf("unauthenticated: %w", err)
	}
	...
	err = r.Resolver.FileService.RestoreFolder(user.ID, uint(fID))
	if err != nil {
		return false, err
	}
...
// PermanentlyDeleteFolder is the resolver for the permanentlyDeleteFolder field.
func (r *mutationResolver) PermanentlyDeleteFolder(ctx context.Context, folderID string) (bool, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return false, fmt.Errorf("unauthenticated: %w", err)
	}
	...
	err = r.Resolver.FileService.PermanentlyDeleteFolder(user.ID, uint(fID))
	if err != nil {
		return false, err
	}
...
func (r *mutationResolver) StarFile(ctx context.Context, id string) (bool, error) {
	...
	log.Printf("DEBUG: StarFile called for user %d, file %d", user.ID, userFileID)
	err = r.Resolver.FileService.StarFile(user.ID, uint(userFileID))
	if err != nil {
		log.Printf("DEBUG: StarFile failed: %v", err)
		return false, err
	}

	log.Printf("DEBUG: StarFile succeeded for user %d, file %d", user.ID, userFileID)
	return true, nil
}
...
// UpdateRoomMemberRole is the resolver for the updateRoomMemberRole field.
func (r *mutationResolver) UpdateRoomMemberRole(ctx context.Context, input model.UpdateRoomMemberRoleInput) (bool, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return false, fmt.Errorf("unauthenticated: %w", err)
	}
	...
	targetUserID, err := strconv.ParseUint(input.UserID, 10, 32)
	if err != nil {
	...
	err = r.Resolver.RoomService.UpdateRoomMemberRole(uint(roomID), uint(targetUserID), user.ID, role)
	if err != nil {
		return false, err
	}
...
func (r *mutationResolver) LeaveRoom(ctx context.Context, roomID string) (bool, error) {
	...
	err = r.Resolver.RoomService.LeaveRoom(uint(rID), user.ID)
	if err != nil {
		return false, err
	}
...
func (r *mutationResolver) CreateFileShare(ctx context.Context, input model.CreateFileShareInput) (*models.FileShare, error) {
	...
	if input.MasterPassword != nil {
		fmt.Printf("DEBUG: CreateFileShare input.MasterPassword = '%s'\n", *input.MasterPassword)
	} else {
		fmt.Printf("DEBUG: CreateFileShare input.MasterPassword = nil (passwordless share)\n")
	}
	fmt.Printf("DEBUG: CreateFileShare input.AllowedEmails = %+v (type: %T)\n", input.AllowedEmails, input.AllowedEmails)
	...
	fileShare, err := r.Resolver.ShareService.CreateShare(uint(userFileID), masterPassword, maxDownloads, input.ExpiresAt, input.AllowedEmails)
	if err != nil {
...
// UpdateFileShare is the resolver for the updateFileShare field.
func (r *mutationResolver) UpdateFileShare(ctx context.Context, input model.UpdateFileShareInput) (*models.FileShare, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("unauthenticated: %w", err)
	}

	shareID, err := strconv.ParseUint(input.ShareID, 10, 32)
	if err != nil {
	...
	updatedShare, err := r.Resolver.ShareService.UpdateShare(
		user.ID,
		uint(shareID),
		input.MasterPassword,
		input.MaxDownloads,
...
func (r *mutationResolver) AccessSharedFile(ctx context.Context, input model.AccessSharedFileInput) (string, error) {
	...
	// Try to get user from context (could be nil for anonymous access)
	...
	// Check allowed emails authorization if user is authenticated and allowed_emails is not empty
	if user != nil && fileShare.AllowedEmails != "" && fileShare.AllowedEmails != "[]" {
		var allowedEmails []string
		if err := json.Unmarshal([]byte(fileShare.AllowedEmails), &allowedEmails); err != nil {
			r.Resolver.ShareService.LogFailedDownload(fileShare.ID, attempt, "failed to parse allowed emails")
			return "", fmt.Errorf("failed to parse allowed emails")
		}
		if len(allowedEmails) > 0 {
			...
			if !authorized {
				r.Resolver.ShareService.LogFailedDownload(fileShare.ID, attempt, "user not in allowed list")
				return "", fmt.Errorf("access denied: your email is not in the allowed list for this share")
			}
		}
	}

	// Validate password (skip for passwordless shares)
	var decryptedKey []byte
	if fileShare.PlainTextPassword != "" {
		// Password required - validate input
		if input.MasterPassword == nil || *input.MasterPassword == "" {
			r.Resolver.ShareService.LogFailedDownload(fileShare.ID, attempt, "password required but not provided")
			return "", fmt.Errorf("password is required for this share")
		}
		var err error
		...
		// Passwordless share - use stored password for decryption
		var err error
		decryptedKey, err = r.Resolver.ShareService.DecryptFileKey(fileShare, "")
		if err != nil {
			r.Resolver.ShareService.LogFailedDownload(fileShare.ID, attempt, "failed to decrypt passwordless share")
			return "", fmt.Errorf("failed to access passwordless share")
		}
	}
	...
	// Record user access for "Shared with Me" functionality (if user is logged in)
	if userID != nil {
		err = r.Resolver.ShareService.RecordUserAccess(userID, fileShare.ID, input.Token, ipAddress, userAgent)
		if err != nil {
			// Log error but don't fail the request
			log.Printf("Failed to record user access: %v", err)
		}
	}
...
func (r *mutationResolver) UpdateProfile(ctx context.Context, input model.UpdateProfileInput) (*models.User, error) {
	...
	// Convert pointers to strings, using empty string for nil values
	username := ""
	...
	updatedUser, err := r.Resolver.UserService.UpdateProfile(
		user.ID,
		username,
		email,
		currentPassword,
...
func (r *queryResolver) MyStarredFiles(ctx context.Context) ([]*models.UserFile, error) {
	...
	files, err := r.Resolver.FileService.GetStarredFiles(user.ID)
	log.Printf("DEBUG: MyStarredFiles returned %d files for user %d", len(files), user.ID)
	return files, err
}

// MyStarredFolders is the resolver for the myStarredFolders field.
func (r *queryResolver) MyStarredFolders(ctx context.Context) ([]*models.Folder, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("unauthenticated: %w", err)
	}

	log.Printf("DEBUG: MyStarredFolders called for user %d", user.ID)
	folders, err := r.Resolver.FileService.GetStarredFolders(user.ID)
	log.Printf("DEBUG: MyStarredFolders returned %d folders for user %d", len(folders), user.ID)
	return folders, err
}
...
// MyTrashedFolders is the resolver for the myTrashedFolders field.
func (r *queryResolver) MyTrashedFolders(ctx context.Context) ([]*models.Folder, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("unauthenticated: %w", err)
	}

	log.Printf("DEBUG: MyTrashedFolders called for user %d", user.ID)

	// Get all folders including deleted ones for this user
	trashedFolders, err := r.Resolver.FileService.GetTrashedFolders(user.ID)
	if err != nil {
		return nil, err
	}

	log.Printf("DEBUG: Found %d trashed folders", len(trashedFolders))
	return trashedFolders, nil
}
...
func (r *queryResolver) Users(ctx context.Context, search *string) ([]*models.User, error) {
	_, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("unauthenticated: %w", err)
	}
	...
	return r.Resolver.UserService.SearchUsers(searchTerm)
}
...
func (r *queryResolver) MyShares(ctx context.Context) ([]*models.FileShare, error) {
	...
	// Use ShareService to get user shares
	shares, err := r.Resolver.ShareService.GetUserShares(user.ID)
	if err != nil {
...
func (r *queryResolver) SharedWithMe(ctx context.Context) ([]*model.SharedWithMeFile, error) {
	user, err := middleware.GetUserFromContext(ctx)
	if err != nil {
		return nil, fmt.Errorf("unauthorized: %w", err)
	}

	// Get shared files accessed by this user
	sharedFiles, err := r.Resolver.ShareService.GetSharedWithMeFiles(user.ID)
	if err != nil {
		return nil, fmt.Errorf("failed to get shared files: %w", err)
	}

	// Convert service types to GraphQL model types
	var result []*model.SharedWithMeFile
	for _, sf := range sharedFiles {
		result = append(result, &model.SharedWithMeFile{
			ID:            sf.ID,
			...
			FirstAccessAt: sf.FirstAccessAt,
			LastAccessAt:  sf.LastAccessAt,
...
func (r *queryResolver) ShareMetadata(ctx context.Context, token string) (*model.ShareMetadata, error) {
	metadata, err := r.Resolver.ShareService.GetShareMetadata(token)
	if err != nil {
		return nil, err
	}
	...
	return &model.ShareMetadata{
		...
		SizeBytes:        int(metadata.SizeBytes),
...
// ID is the resolver for the id field.
func (r *sharedFileAccessResolver) ID(ctx context.Context, obj *models.SharedFileAccess) (string, error) {
	panic(fmt.Errorf("not implemented: ID - id"))
}

// UserID is the resolver for the user_id field.
func (r *sharedFileAccessResolver) UserID(ctx context.Context, obj *models.SharedFileAccess) (*string, error) {
	panic(fmt.Errorf("not implemented: UserID - user_id"))
}

// FileShareID is the resolver for the file_share_id field.
func (r *sharedFileAccessResolver) FileShareID(ctx context.Context, obj *models.SharedFileAccess) (string, error) {
	panic(fmt.Errorf("not implemented: FileShareID - file_share_id"))
}
...
// SharedFileAccess returns generated.SharedFileAccessResolver implementation.
func (r *Resolver) SharedFileAccess() generated.SharedFileAccessResolver {
	return &sharedFileAccessResolver{r}
}
...
type sharedFileAccessResolver struct{ *Resolver }

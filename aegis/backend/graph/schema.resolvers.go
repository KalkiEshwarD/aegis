...
func (r *mutationResolver) CreateFileShare(ctx context.Context, input model.CreateFileShareInput) (*models.FileShare, error) {
	...
	fileShare, err := r.Resolver.ShareService.CreateShare(uint(userFileID), masterPassword, maxDownloads, input.ExpiresAt, input.AllowedUsernames)
	if err != nil {
...
func (r *mutationResolver) AccessSharedFile(ctx context.Context, input model.AccessSharedFileInput) (string, error) {
	...
	// Check allowed usernames authorization if user is authenticated and allowed_usernames is not empty
	if user != nil && len(fileShare.AllowedUsernames) > 0 {
		authorized := false
		...
		if !authorized {
			...
			return "", fmt.Errorf("access denied: your username is not in the allowed list for this share")
		}
	}
